...
def test_tag_file__multiple__ok(client, generate_csv, db_session):
    ...
    response = client.post("/files/test_file.csv/tags", params={"tag_name": "second_tag"})
    file_tags = db_session.exec(select(FileTag).where(FileTag.file_id == 1)).all()
...
def test_tag_file__file_not_found(client):
    response = client.post("/files/invalid_file.csv/tags", params={"tag_name": "test_tag"})
...
def test_tag_file__conflict(client, generate_csv, db_session):
    ...
    assert response.json()["detail"] == "File 'test_file.csv' already has 'test_tag' tag"
...
def test_untag_file__ok(client, generate_csv, db_session):
    ...
    response = client.delete("/files/test_file.csv/tags", params={"tag_name": "test_tag"})

    assert response.status_code == 200

def test_untag_file__file_not_found(client):
    response = client.delete("/files/invalid_file.csv/tags", params={"tag_name": "test_tag"})
...
def test_untag_file__conflict(client, generate_csv, db_session):
    ...
    assert response.status_code == 409
    assert response.json()["detail"] == "File 'test_file.csv' does not have 'test_tag' tag"
